...
def validate_long_sc_dataset(dataset_path):
    ...
    print("\n" + "="*50)
    print("LONG SC VALIDATION RESULTS")
    print("="*50)
    ...
    with open('results/long_sc_validation_metrics.json', 'w') as f:
        json.dump({
            'overall': metrics,
            'class_report': report
        }, f, indent=4)
    
    print(f"\nResults saved to results/long_sc_validation_metrics.json")
    
    return metrics, df_results

def main():
    parser = argparse.ArgumentParser(description='Validate binary classifier on Long SC dataset')
    parser.add_argument('--dataset', type=str, default='results/long_sc_valid_results_20250407_000413_analyzed.json',
                        help='Path to the Long SC dataset JSON file')
