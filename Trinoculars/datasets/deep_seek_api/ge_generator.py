...
def generate_paragraph(prompt, temperature=0.8):
    try:
        completion = client.chat.completions.create(
            extra_headers={
                "HTTP-Referer": "<YOUR_SITE_URL>",  # Optional. Site URL for openrouter.ai rankings
                "X-Title": "<YOUR_SITE_NAME>",      # Optional. Site name for openrouter.ai rankings
            },
...
def create_dataset(num_paragraphs=10, output_file="dataset.json"):
    ...
    for i in tqdm(range(num_paragraphs), desc="Generating paragraphs", unit="paragraph"):
        ...
        entry = {
            ...
            "text": text,
            "source": "ai",
            "dataset": "gemini-2.0-flash-lite-preview-02-05"
        }
...
if __name__ == "__main__":
    create_dataset(num_paragraphs=1000, output_file="small_dataset_ge.json")
